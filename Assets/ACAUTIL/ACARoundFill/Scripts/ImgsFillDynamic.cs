...
public class ImgsFillDynamic : MonoBehaviour
{
    ...
    void ShakeUI(float facter)
    {
        float normalized = Mathf.InverseLerp(0.6f, 1f, facter);
        float frequency = Mathf.Lerp(10f, 50f, normalized);  //frequency  
        float amplitude = Mathf.Lerp(1f, 4f, normalized);   //amplitude   

        float shakeOffset = Mathf.Sin(Time.time * frequency) * amplitude;
        Vector3 offset = new Vector3(shakeOffset, 0f, 0f);
        this.transform.localPosition = originalPos + offset;
    }
